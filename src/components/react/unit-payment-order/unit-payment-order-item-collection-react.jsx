...
import UnitPaymentOrderItemReact from './unit-payment-order-item-react.jsx';
'use strict';

export default class UnitPaymentOrderItemCollectionReact extends React.Component {
    constructor(props) {
        super(props);
    ...
    render() {
        ...
        var items = (this.state.value || []).map((unitReceiptNote, index) => {
            var error = this.state.error[index] || {};
            return (
                <UnitPaymentOrderItemReact key={"__item" + index} value={unitReceiptNote} error={error} options={this.state.options} onChange={this.handleItemChange} onItemRemove={this.handleItemRemove}></UnitPaymentOrderItemReact>
            );
        })
