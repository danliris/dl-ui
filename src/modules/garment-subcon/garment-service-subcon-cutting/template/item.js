...
const CuttingInLoader = require("../../../../loader/garment-cutting-in-by-ro-currentyear-loader");
    ...
    if (this.isCreate) {
      if (newValue) {
        ...
        if (noResult.data.length > 0) {
          ...
          const comodityCodeResult = await this.salesService.getHOrderKodeByNo({
            no: this.data.RONo,
          });
          const comodityCode = comodityCodeResult.data[0];
          if (comodityCode) {
            const comodityResult = await this.coreService.getGComodity({
              size: 1,
              filter: JSON.stringify({ Code: comodityCode }),
            });
            this.data.Comodity = comodityResult.data[0];
          }
        }

        //Old Query Calculating Recent item that uses, from items to cutIn (13/09/2023)
        // let ssCuttingItems=[];
          ...
          for (var cuttingInHeader of result.data) {
            for (var cuttingInItem of cuttingInHeader.Items) {
              for (var cuttingInDetail of cuttingInItem.Details) {
                var detail = {};
                if (this.data.Details.length == 0) {
                  detail.CuttingInQuantity = cuttingInDetail.CuttingInQuantity;
                  detail.DesignColor = cuttingInDetail.DesignColor;
                  this.data.Details.push(detail);
                } else {
                  var exist = this.data.Details.find(
                    (a) => a.DesignColor == cuttingInDetail.DesignColor
                  );
                  if (!exist) {
                    detail.CuttingInQuantity =
                      cuttingInDetail.CuttingInQuantity;
                    detail.DesignColor = cuttingInDetail.DesignColor;
                    this.data.Details.push(detail);
                  } else {
                    var idx = this.data.Details.indexOf(exist);
                    exist.CuttingInQuantity +=
                      cuttingInDetail.CuttingInQuantity;
                    this.data.Details[idx] = exist;
                  }
                }
