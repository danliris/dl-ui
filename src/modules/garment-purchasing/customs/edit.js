...
export class Edit {
  ...
  async activate(params) {
    ...
    var currencyCode = this.data.currency.code
      ...
      : this.data.currency.Code;
    //Get Data DO
    var dataDelivery = await this.service.searchDeliveryOrder({
      supplier: `${supplierId}`,
    ...
    if (this.data.customType == "BC 262") {
      dataDeliveryNonPO = await this.service.searchDeliveryOrderNonPO({
        supplier: `${supplierId}`,
        currency: `${currencyCode}`,
      });
    }
    if (this.data.customType == "BC 40" || this.data.customType == "BC 27") {
      dataDeliveryNonPO = await this.service.searchSubconDeliveryOrder({
        supplier: `${supplierId}`,
        currency: `${currencyCode}`,
      });
    }
    ...
    this.data.deliveryOrders = this.data.items.filter(
      (x) => x.deliveryOrderNonPO == null
    );
    this.data.deliveryOrderNonPO = this.data.items.filter(
      (x) => x.deliveryOrderNonPO != null
    );
    ...
    this.data.validateDate = moment(this.data.validateDate).format(
      "YYYY-MM-DD"
    );
  }
  ...
  save(event) {
    ...
    if (this.data.customsDate && this.data.customsDate !== "")
      this.data.customsDate = moment(this.data.customsDate).format(
        "YYYY-MM-DD"
      );
    ...
    if (dataCustoms.deliveryOrders && dataCustoms.deliveryOrders.length > 0) {
      ...
      dataCustoms.deliveryOrders = items.filter(
        (x) => x.deliveryOrder.IsPO == true
      );
    }

    //Mapping DO Non PO
    if (
      dataCustoms.deliveryOrderNonPO &&
      dataCustoms.deliveryOrderNonPO.length > 0
    ) {
      ...
      dataCustoms.deliveryOrderNonPO = items.filter(
        (x) => x.deliveryOrder.IsPO == false
      );
    }
      ...
      this.service
        ...
        .catch((e) => {
          ...
          if (e.deliveryOrders.length > 0) {
            ...
            for (var a of this.data.deliveryOrders) {
              var error = {};
              var item = e.deliveryOrders.find(
                (dataItem) => dataItem.dOrderNumber === a.no
              );
              if (item) error["no"] = item.no;
