    ...
    if (!validationError) {
      ...
      this.data = await this.service.search(params).then((result) => {
        ...
        if (result.data && result.data.length > 0) {
          ...
          this.initialBalance = numeral(result.data[0].BeforeNominal).format(
            "0,0.00"
          );
          this.closingBalance = numeral(
            result.data[result.data.length - 1].AfterNominal
          ).format("0,0.00");
        }
        let index = 0;
        for (let data of result.data) {
          ...
          let dataSet = {
            ...
            AccountBankCurrencyCode: data.AccountBankCurrencyCode,
            Debit:
              data.Status.toString().toLowerCase() == "in"
                ? numeral(data.Nominal).format("0,0.00")
                : "",
            Kredit:
              data.Status.toString().toLowerCase() == "out"
                ? numeral(data.Nominal).format("0,0.00")
                : "",
          ...
          index++;
          if (
            !resultDataSet[resultDataSet.length - 1].DailyTotalTitle &&
            index == result.data.length
          ) {
            let dailyTotalDataSet = {
              DailyTotalTitle: "Total Harian",
