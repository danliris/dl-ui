...
const SalesContractLoader = require('../../../../loader/garment-shipping-export-sales-contract-loader');
...
export class DataForm {
    ...
    async bind(context) {
        ...
        if (this.data.id) {
            ...
            this.selectedSalesContract = {
                ...
                id: this.data.exportSalesContractId
            }
            if (this.data.paymentType) {
                this.selectedPaymentType = this.data.paymentType;
            }
            var sc = await this.service.getSCById(this.data.exportSalesContractId);
            for (var item of this.data.items) {
                var scItem = sc.items.find(a => a.id == item.exportSalesContractItemId);
                //console.log(sc,item)
                if (scItem) {
    ...
    async selectedSalesContractChanged(newValue, oldValue) {
        if (!this.data.id)
            this.data.items.splice(0);
        if (newValue) {
            if (newValue.id != this.data.exportSalesContractId) {
                console.log(this.data);
                this.data.exportSalesContractId = newValue.id;
                ...
                if (!this.data.id) {
                    ...
                    for (var a of sc.items) {
                        ...
                        if (a.remainingQuantity > 0) {
                            item.exportSalesContractItemId = a.id;
            ...
            this.data.exportSalesContractId = 0;
        }
    }
