...
export class DataForm {
 ...
 // @bindable selectedWarehouse;
  ...
  async attached() {
    console.log(this.data.dyeingPrintingWarehouseInItems);
    if (this.data.dyeingPrintingWarehouseInItems != undefined) {
  ...
  async barcodeManual() {
      ...
      if (temp.length != 0){
        if(temp != undefined) {
          if(Object.getOwnPropertyNames(temp).length > 0) {
            ...
            console.log(this.data);
            console.log(temp1);
            var data = this.data.dyeingPrintingWarehouseInItems.find((x) => x.productPackingCode === temp1.productPackingCode);
            console.log(data);
            if(!data) {
              this.data.dyeingPrintingWarehouseInItems.push(temp1);
              //this.data.DyeingPrintingStockOpnameMutationItems.push(temp1);
            ...
            this.makeTotal(this.data.dyeingPrintingWarehouseInItems);
          }
        }
    ...
    async qtyChange(code, qty, length) {
      ...
      if (quantity != undefined) {
          ...
          // var newItem = {};
          console.log(this.data.dyeingPrintingWarehouseInItems);
          var _data = this.data.dyeingPrintingWarehouseInItems.find((item) => item.productPackingCode === barcode);
  ...
  removeItem(item) {
    var itemIndex = this.data.dyeingPrintingWarehouseInItems.indexOf(item);

    console.log(this.data);
    this.data.dyeingPrintingWarehouseInItems.splice(itemIndex, 1);
    // if(this.error)
