...
export class List {
  ...
  async getcomodity(newValue) {
    ...
    let noResult = await this.salesService.getCostCalculationByRONo({
      size: 1,
      filter: JSON.stringify({ RO_Number: this.RONo }),
    });
  ...
  searching() {
    ...
    if (!this.RONo) {
      ...
      this.service
        .searchPreparing({ filter: JSON.stringify({ RONo: this.RONo }) })
        .then((result) => {
          this.data = [];
          this.products = [];
          ...
          for (var prepare of result.data) {
            ...
            for (var item of prepare.Items) {
              ...
              dataItem[item.Product.Code] = dataItem[item.Product.Code]
                ? dataItem[item.Product.Code] + item.Quantity
                : item.Quantity;
          ...
          this.service
            .searchCutting({ filter: JSON.stringify({ RONo: this.RONo }) })
            .then((result) => {
              this.cuttingData = [];
              this.cuttingSizes = [];
                ...
                for (var item of cutting.Items) {
                  for (var detail of item.Details) {
                    ...
                    if (this.cuttingData.length == 0) {
                      ...
                      dataItem[detail.Size.Size] = dataItem[detail.Size.Size]
                        ? dataItem[detail.Size.Size] + detail.CuttingOutQuantity
                        : detail.CuttingOutQuantity;
                      this.cuttingData.push(dataItem);
                    } else {
                      var same = this.cuttingData.find(
                        (s) =>
                          s.color == detail.Color &&
                          s.CuttingNo == cutting.CutOutNo
                      );
                      if (!same) {
                        dataItem["color"] = detail.Color;
                        dataItem["CuttingNo"] = cutting.CutOutNo;
                        ...
                        same[detail.Size.Size] = same[detail.Size.Size]
                          ? same[detail.Size.Size] + detail.CuttingOutQuantity
                          : detail.CuttingOutQuantity;
                    ...
                    if (this.cuttingSizes.length == 0) {
                      ...
                      var dup = this.cuttingSizes.find(
                        (a) => a == detail.Size.Size
                      );
                      if (!dup) {
                        this.cuttingSizes.push(detail.Size.Size);
                      }
              ...
              this.service
                .searchLoading({ filter: JSON.stringify({ RONo: this.RONo }) })
                .then((result) => {
                  this.loadingData = [];
                  this.loadingSizes = [];
                  ...
                  for (var loading of result.data) {
                    for (var detail of loading.Items) {
                      ...
                      if (this.loadingData.length == 0) {
                        dataItem["color"] = detail.Color;
                        dataItem["loadingNo"] = loading.LoadingOutNo;
                        dataItem["date"] = loading.LoadingDate;
                        dataItem["qty"] = detail.Quantity;
                        dataItem[detail.Size.Size] = dataItem[detail.Size.Size]
                          ? dataItem[detail.Size.Size] + detail.Quantity
                          : detail.Quantity;
                        this.loadingData.push(dataItem);
                      } else {
                        var same = this.loadingData.find(
                          (s) =>
                            s.color == detail.Color &&
                            s.loadingNo == loading.LoadingNo
                        );
                        if (!same) {
                          dataItem["color"] = detail.Color;
                          dataItem["loadingNo"] = loading.LoadingOutNo;
                          dataItem["date"] = loading.LoadingDate;
                          dataItem["qty"] = detail.Quantity;
                          dataItem[detail.Size.Size] = dataItem[
                            detail.Size.Size
                          ]
                            ? dataItem[detail.Size.Size] + detail.Quantity
                            : detail.Quantity;
                          ...
                          same[detail.Size.Size] = same[detail.Size.Size]
                            ? same[detail.Size.Size] + detail.Quantity
                            : detail.Quantity;
                      ...
                      if (this.loadingSizes.length == 0) {
                        ...
                        var dup = this.loadingSizes.find(
                          (a) => a == detail.Size.Size
                        );
                        if (!dup) {
                          this.loadingSizes.push(detail.Size.Size);
                    ...
                    .then((result) => {
                        ...
                        for (var item of sewing.Items) {
                          ...
                          if (this.sewingData.length == 0) {
                            ...
                            if (sewing.IsDifferentSize) {
                              ...
                              dataItem["qty"] += item.Quantity;
                              dataItem[item.Size.Size] = dataItem[
                                item.Size.Size
                              ]
                                ? dataItem[item.Size.Size] + item.Quantity
                                : item.Quantity;
                            }
                            this.sewingData.push(dataItem);
                          } else {
                            var same = this.sewingData.find(
                              (s) =>
                                s.color == item.Color &&
                                s.sewingNo == sewing.SewingOutNo
                            );
                            if (!same) {
                              dataItem["color"] = item.Color;
                              dataItem["sewingNo"] = sewing.SewingOutNo;
                              ...
                              if (sewing.IsDifferentSize) {
                                ...
                                same[item.Size.Size] = same[item.Size.Size]
                                  ? same[item.Size.Size] + item.Quantity
                                  : item.Quantity;
                              }
                      ...
                      this.service
                        ...
                        .then((result) => {
                          ...
                          for (var finishing of result.data) {
                            for (var item of finishing.Items) {
                              if (finishing.IsDifferentSize) {
                                for (var detail of item.Details) {
                                  if (this.finishingSizes.length == 0) {
                                    ...
                                    var dup = this.finishingSizes.find(
                                      (a) => a == detail.Size.Size
                                    );
                                    if (!dup) {
                                      this.finishingSizes.push(
                                        detail.Size.Size
                                      );
                                    }
                                  }
                              ...
                              if (this.finishingData.length == 0) {
                                dataItem["color"] = item.Color;
                                dataItem["finishingNo"] =
                                  finishing.FinishingOutNo;
                                ...
                                var same = this.finishingData.find(
                                  (s) =>
                                    s.color == item.Color &&
                                    s.finishingNo == finishing.FinishingOutNo
                                );
                                if (!same) {
                                  dataItem["color"] = item.Color;
                                  dataItem["finishingNo"] =
                          ...
                          this.service
                            .searchExpenditure({
                              filter: JSON.stringify({ RONo: this.RONo }),
                            })
                            .then((result) => {
                              ...
                              for (var expenditure of result.data) {
                                for (var item of expenditure.Items) {
                                  ...
                                  if (this.expenditureData.length == 0) {
                                    ...
                                    dataItem["expenditureNo"] =
                                      expenditure.PackingOutNo;
                                    dataItem["date"] =
                                    ...
                                    var same = this.expenditureData.find(
                                      (s) =>
                                        s.color == item.Description &&
                                        s.expenditureNo ==
                                          expenditure.PackingOutNo
                                    );
                                    if (!same) {
                                      ...
                                      dataItem[item.Size.Size] = dataItem[
                                        ...
                                        ? dataItem[item.Size.Size] +
                                          item.Quantity
                                        : item.Quantity;
                                      this.expenditureData.push(dataItem);
                                    } else {
                                      var idx =
                                        this.expenditureData.indexOf(same);
                                      same["qty"] += item.Quantity;
                                      same[item.Size.Size] = same[
                                        item.Size.Size
                                      ]
                                        ? same[item.Size.Size] + item.Quantity
                                        : item.Quantity;
                                  ...
                                  if (this.expenditureSizes.length == 0) {
                                    ...
                                    var dup = this.expenditureSizes.find(
                                      (a) => a == item.Size.Size
                                    );
                                    if (!dup) {
                                      this.expenditureSizes.push(
                                        item.Size.Size
                                      );
                                    }
                                  }
                                  this.totalExpenditureQuantity +=
                                    item.Quantity;
                                }
                              }
  ...
  fillTable() {
    ...
    $(this.cuttingTable)
      .bootstrapTable("destroy")
      .bootstrapTable(bootstrapCuttingTableOptions);
    ...
    $(this.loadingTable)
      .bootstrapTable("destroy")
      .bootstrapTable(bootstrapLoadingTableOptions);
    ...
    $(this.expenditureTable)
      .bootstrapTable("destroy")
      .bootstrapTable(bootstrapExpenditureTableOptions);
  }
