...
export class Detail {
  ...
  activate(context) {
    ...
    this.itemOptions = {
      ...
      isEdit: this.isEdit,
      item: context.context.options.item,
    };
  ...
  get addSizes() {
    return (event) => {
      this.data.sizes.push({
        // DetailSize: this.data.sizePackingOut,
  ...
  get totalQuantity() {
    if (this.data.cartonQuantity && this.data.quantityPCS) {
      this.data.totalQuantity =
        this.data.cartonQuantity * this.data.quantityPCS;
  ...
  sumSubTotal(opt) {
    let result = 0;
    const newDetails = this.context.context.options.item.details
      .map((d) => {
        return {
          carton1: d.carton1,
          carton2: d.carton2,
    ...
    for (const detail of newDetails) {
      ...
      const indexItem = this.context.context.options.header.items.indexOf(
        this.context.context.options.item
      );
      if (indexItem > 0) {
  ...
  indexChanged(newValue) {
    this.context.context.options.item.details[
      this.context.context.options.item.details.length - 1
    ].index = newValue;
  }
}
