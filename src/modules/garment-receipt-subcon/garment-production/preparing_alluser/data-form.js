...
var UENLoader = require("../../../../loader/subcon-unit-expenditure-note-gpreparing-loader");
...
export class DataForm {
  ...
  bind(context) {
    ...
    if (this.data) {
      ...
      this.data.buyerView = this.data.Buyer
    ...
    if (this.options.isView) {
      this.purchasingService
        .getUnitExpenditureNoteById(this.dataView.UENId)
        .then((uenNo) => {
          this.uenNo = uenNo;
        });
    }
        ...
        .then((deliveryOrder) => {
          if (deliveryOrder) {
            ...
            this.purchasingService
              ...
              .then((pr) => {
                if (pr.data.length > 0) {
                  this.data.Buyer = pr.data[0].Supplier;
                  this.data.buyerView =
                    this.data.Buyer.Code + "-" + this.data.Buyer.Name;
                }
                ...
                for (var doItem of deliveryOrder.Items) {
                  for (var item of this.data.Items) {
                    ...
                    item.BeacukaiNo = item.BeacukaiNo;
                    item.BeacukaiType = item.BeacukaiType;
                    item.BeacukaiDate = item.BeacukaiDate;
                    var existDOItem = deliveryOrder.Items.find(
                      (x) => x.Id == item.UnitDOItemId
                    );
                    if (existDOItem) {
                      item.ROSource = existDOItem.RONo;
                      item.DesignColor = existDOItem.DesignColor;
                    }
                  }
                }
              });
