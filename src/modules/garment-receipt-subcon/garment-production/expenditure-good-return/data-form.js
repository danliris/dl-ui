...
export class DataForm {
    ...
    return (keyword) => {
      var info = {
        ...
        filter: JSON.stringify({
          ...
          PackingOutType: "LOKAL",
        }),
      };
      // if (keyword.length >= 5) {
      return this.service.getExpenditureGoodByNo(info).then((result) => {
        var roList = [];
        for (var a of result.data) {
          if (roList.length == 0) {
            roList.push(a);
          } else {
            var dup = roList.find((d) => d.PackingOutNo == a.PackingOutNo);
            if (!dup) {
              roList.push(a);
            }
          }
  ...
  get GBCNoLoader() {
    return (keyword) => {
      ...
      return this.customReportService.getBCNo(info).then((result) => {
  ...
  async selectedEGChanged(newValue, oldValue) {
    ...
    if (newValue) {
      ...
      this.data.Buyer = newValue.ProductOwner;
      ...
      Promise.resolve(
        this.service.getExpenditureGood({
          filter: JSON.stringify({
          ...
          for (var exGoodItem of exGood.Items) {
            ...
            if (Qty > 0) {
              ...
              if (this.data.Items.length > 0) {
                var duplicate = this.data.Items.find(
                  (a) =>
                    a.Size.Id == exGoodItem.Size.Id &&
                    a.Uom.Id == exGoodItem.Uom.Id &&
                    a.Description == exGoodItem.Description
                );
                ...
                if (duplicate) {
                  ...
                  item.IsSave = true;
                  item.PackingOutDate = exGood.PackingOutDate;
  ...
  itemsInfoNotCreate = {
    columns: ["Size", "Jumlah Retur", "Satuan", "Keterangan"],
  };

  EGView = (eg) => {
    return `${eg.PackingOutNo}`;
  };
