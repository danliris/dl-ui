...
export class DataForm {
  ...
  finishingToOptions = ["PACKING", "AVAL"];
  ...
  async selectedROChanged(newValue, oldValue) {
    if (this.context.isCreate) {
      if (newValue) {
        ...
        Promise.resolve(
          this.service.searchFinishingInComplete({
            filter: JSON.stringify({
              RONo: this.data.RONo,
              UnitId: this.data.Unit.Id,
          ...
          for (var finishingIn of result.data) {
            this.data.FinishingInDate = !this.data.FinishingInDate
              ? finishingIn.FinishingInDate
              : finishingIn.FinishingInDate < this.data.FinishingInDate
              ? finishingIn.FinishingInDate
            ...
            for (var finishingInItem of finishingIn.Items) {
              ...
              if (finishingInItem.RemainingQuantity > 0) {
                ...
                item.Quantity = this.data.IsDifferentSize
                  ? finishingInItem.RemainingQuantity
                  : 0;
  ...
  get roLoader() {
    return (keyword) => {
      ...
      return this.service.searchFinishingInByRo(info).then((result) => {
        var roList = [];
