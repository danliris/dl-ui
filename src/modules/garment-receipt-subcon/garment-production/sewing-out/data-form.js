...
export class DataForm {
  ...
  sewingToOptions = ["FINISHING", "PACKING", "AVAL"];
  ...
  async selectedROChanged(newValue, oldValue) {
    if (this.context.isCreate) {
      if (newValue) {
        ...
        let pr = await this.service.getPreparings({
          size: 1,
        ...
        Promise.resolve(
          this.service.searchSewingIn({
            filter: JSON.stringify({
              RONo: this.data.RONo,
              UnitId: this.data.Unit.Id,
              "GarmentSewingInItem.Any(RemainingQuantity>0)": true,
              IsApproved: true,
            }),
          })
        ).then((result) => {
