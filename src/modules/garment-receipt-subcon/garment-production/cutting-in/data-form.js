...
const PreparingLoader = require("../../../../loader/garment-subcon-preparing-ro-loader");
...
export class DataForm {
  ...
  cuttingTypes = ["Main Fabric", "Non Main Fabric"];
  ...
  get preparingLoader() {
    return (keyword) => {
      ...
      return this.service.getPreparingByRo(info).then((result) => {
        var roList = [];
  ...
  async selectedPreparingChanged(newValue, oldValue) {
    if (newValue) {
      ...
      if (noResult.data.length > 0) {
        ...
        if (comodityCode) {
          const comodityResult = await this.coreService.getComodities({
            size: 1,
        ...
        this.data.Items = result.data
          .map((data) => {
            return Object.assign(data, {
              PreparingId: data.Id,
              Details: data.Items.filter((item) => {
