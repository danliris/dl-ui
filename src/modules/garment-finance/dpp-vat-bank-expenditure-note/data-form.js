...
export class DataForm {
  ...
  bankView = (bank) => {
    return (
      bank.BankName + " " + bank.Currency.Code + " - " + bank.AccountNumber
    );
  };
  ...
  bankAccountChanged(newValue, oldValue) {
    ...
    if (newValue) {
      ...
      if (
        this.bankCurrency == "IDR" &&
        this.currencyCodeValue != "IDR" &&
        this.currencyCodeValue != null &&
        !this.readOnly
      ) {
  ...
  async currencyChanged(newValue, oldValue) {
    ...
    if (newValue) {
      ...
      if (this.supplier && !this.readOnly && this.data.ItemType == "OTOMATIS") {
        let newItems = await this.purchasingService
          ...
          .then((items) => {
            return items.map((item) => {
              item.Id = 0;
              item.InternalNote.Items = item.InternalNote.Items.map(
                (internalNoteItem) => {
        ...
        if (newItems) {
          this.data.Items = this.data.Items.map((item) => {
            var newItem = newItems.find(
              (element) => element.InternalNote.Id == item.InternalNote.Id
            );

            if (newItem) {
              item.InternalNote.Items = item.InternalNote.Items.concat(
                newItem.InternalNote.Items
              );
            }
          ...
          let items = newItems.filter((element) => {
            let exist = this.data.Items.find(
              (item) => item.InternalNote.Id == element.InternalNote.Id
            );
  ...
  async supplierChanged(newValue, oldValue) {
    ...
    if (newValue && this.data.ItemType == "OTOMATIS") {
