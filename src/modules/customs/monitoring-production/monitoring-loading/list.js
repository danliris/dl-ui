...
export class List {
    ...
    if (Object.getOwnPropertyNames(this.error).length === 0) {
      ...
      this.service.search(info).then((result) => {
        ...
        for (var _data of result.data) {
          _data.TotalLoadingsQuantity =
            _data.TotalLoadingQuantity.toLocaleString("en-EN", {
          ...
          _data.LoadingDate = moment(_data.LoadingDate).format("DD-MMM-YYYY");
  ...
  get sumTotalLoadingQuantity() {
    var sum = 0;
    if (this.data) {
      for (var item of this.data) {
        sum += item.TotalLoadingQuantity;
      }
    }
