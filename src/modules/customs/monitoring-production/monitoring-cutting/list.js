...
export class List {
    ...
    if (Object.getOwnPropertyNames(this.error).length === 0) {
      ...
      this.service.search(info).then((result) => {
        ...
        for (var _data of result.data) {
          _data.TotalCuttingOutsQuantity =
            _data.TotalCuttingOutQuantity.toLocaleString("en-EN", {
          ...
          _data.CuttingOutDate = moment(_data.CuttingOutDate).format(
            "DD-MMM-YYYY"
          );
  ...
  get sumTotalCuttingOutQuantity() {
    var sum = 0;
    if (this.data) {
      for (var item of this.data) {
        sum += item.TotalCuttingOutQuantity;
      }
    }
