...
export class Item {
  ...
  async selectedUENChanged(newValue, oldValue) {
    if (newValue.UENNo != this.data.UnitExpenditureNo) {
      ...
      newValue.Items.map(async (i) => {
        const dataUnitDOItem =
          await this.purchasingService.getUnitDeliveryOrderItems(
            i.UnitDOItemId
          );
        const detail = {};
        detail.Product = {
  ...
  get garmentUENLoader() {
    return async (keyword) => {
      ...
      //add Used UENNo to filter
      let dataSampleShrinkage = await this.service.searchComplete({});
      let uenNo = [];
      dataSampleShrinkage.data.map((x) => {
        x.Items.map((item) => {
      ...
      this.context.context.items.map((x) => {
        uenNo.push(x.data.UnitExpenditureNo);
      });
