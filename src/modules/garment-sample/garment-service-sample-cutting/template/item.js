...
const CuttingInLoader = require("../../../../loader/garment-sample-cutting-in-by-ro-currentyear-loader");
  ...
  async selectedCuttingInChanged(newValue, oldValue) {
    if (this.isCreate) {
      if (newValue) {
        ...
        let noResult = await this.service.getSampleRequest({
          size: 1,
          filter: JSON.stringify({ RONoSample: this.data.RONo }),
        });
