...
export class View {
  ...
  async activate(params) {
    ...
    if (this.data) {
      ...
      let ssCuttingItems = [];
      let ssCutting = await this.service.searchComplete({
        size: 100,
          ...
          if (ssC.Id != this.data.Id) {
            for (var ssCItem of ssC.Items) {
              ...
              if (ssCuttingItems[ssCItem.CuttingInDetailId]) {
                ssCuttingItems[ssCItem.CuttingInDetailId].qty +=
                  ssCItem.Quantity;
              } else {
      ...
      Promise.resolve(
          ...
          for (var cuttingInItem of cuttingInHeader.Items) {
            for (var cuttingInDetail of cuttingInItem.Details) {
              ...
              cuttingInDetail.CuttingInQuantity =
                cuttingInDetail.CuttingInQuantity - qtyOut;
              remainingCuttingInQty[cuttingInDetail.Id] =
                cuttingInDetail.CuttingInQuantity;
            }
          }
        }
        ...
        for (var item of this.data.Items) {
          item.CuttingInQuantity =
            remainingCuttingInQty[item.CuttingInDetailId];
        }
      });
    }
  }
